<template>
  <div class="q-pa-md">
                ...
                <q-td v-for="col in props.cols" :key="col.name" :props="props" :style="{'width': ancho +'px', 'whiteSpace': 'normal'}"  >
                  <div v-if="col.name == 'acciones'" >
                    <q-btn 
                                  ...
                                  size="sm"
                                  color="secondary"
                                  @click="seguimientoTicket(props.row.tic_id)"
                                >
                      <q-tooltip>Crear Seguimiento Ticket: {{props.row.tic_id}}</q-tooltip>
                    </q-btn>
                  </div>
    ...
    <q-dialog v-model="modal_seguimiento" persistent>
      <q-card style="width: 900px; max-width: 70vw">
        <q-bar dark class="bg-primary text-white">
          <div class="col text-center text-weight-bold">
            <div>Seguimiento de Tickets</div>
          </div>
          <q-space />
      ...
      <br>
      <div class="row justify-center">
        <div class="col-md-6 col-sm-6 col-xs-6 q-pa-xs">
         <q-select :dense="true" filled v-model="estadoHistorial" 
            label="Estado *" :options=optionsEstado
            option-value="est_id"
            option-label="est_nombre"
            option-disable="inactive"
      ...
      <div class="row justify-center">
         <div class="col-md-6 col-sm-6 col-xs-6 q-pa-xs">
          <q-input :dense="true" filled v-model="descripcionHistorial" type="textarea" label="Descripción" 
            />
        </div>
      </div>
      <br>
       <div class="row justify-center">
          <div align="center" class="q-gutter-md">
            <q-btn label="Guardar" type="buttom" color="primary" icon="fas fa-save" @click="guardarHistorial"/>
            <q-btn label="Cancelar" type="buttom" color="primary" icon="cancel" @click="!modal_seguimiento"   outline/>
          </div>
        </div>
        <br><br>
        </q-card-section>
...
export default {
  ...
  data(){
    return{
      ...
      estado:'',
      optionsEstado:null,
      modal_seguimiento:null,
      idTicket:null,
      estadoHistorial:null,
      descripcionHistorial:null,
      ...
      columns: [
        ...
        { name: 'name', align: 'center', label: 'Creado Por', field: 'name', sortable: true },
      ...
      var f2;
        if(this.fecha_inicio!='' && this.fecha_fin!=''){
          ...
          try {
              ...
              const json = await axios.get(process.env.API_URL + `gestionar_tickets`,{params});
              this.data=json.data;
          } catch (error) {
            console.log(error);
  ...
  async  seguimientoTicket(idTicket=null){
      this.modal_seguimiento=true;
      ...
      this.$axios.defaults.headers.common['Authorization'] = `Bearer ${token}`;
       const json = await axios.get(process.env.API_URL+`historicoticket/`+idTicket);
        if (json.status === 200) {
  ...
  async  guardarHistorial(){
      ...
      if(this.estadoHistorial!=null){
        const params={
          sgt_id_esc:this.estadoHistorial,
          sgt_comentario:this.descripcionHistorial,
          sgt_id_tic:this.idTicket
        }
        ...
        try {
          ...
          this.$axios.defaults.headers.common['Authorization'] = `Bearer ${token}`;
          const json = await axios.post(process.env.API_URL + `historicoticket`,params);
            if (json.status === 200) {
                  console.log(json.data);
                  this.mostrar_alerta('Registro Almacenado exitosamente');
                  this.modal_seguimiento=false;
                  this.cargarRegistroTickets();
        ...
        this.mostrar_alerta('El campo Estado es requerido',3);
      }
    },
    /*Formatea la fecha de fotmato UTC a DD/MM/YYYY*/
    formatearfecha(fecha){
  ...
  async  cargarRegistroTickets(){
      /*Actuliazamos la tabla general de tickets*/
      /*Consultamos los registros de tickets generados*/
      try {
        let token= this.$store.state.auth.access_token;
        if(token==null){
          token=SessionStorage.getItem("key");
        ...
        this.$axios.defaults.headers.common['Authorization'] = `Bearer ${token}`;
        const json = await axios.get(process.env.API_URL + `get_tickets`);
        this.data=json.data;
      } catch (error) {
        console.log(error);
        if (error.response.status === 401) {
  ...
  computed: {
   ...
   async mounted() {
     ...
     /*Consultamos los registros de tickets generados*/
    await this.cargarRegistroTickets();
